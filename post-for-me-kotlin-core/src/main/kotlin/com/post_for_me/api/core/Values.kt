...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw PostForMeInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw PostForMeInvalidDataException("`$name` is not set")
            is JsonNull -> throw PostForMeInvalidDataException("`$name` is null")
            else -> throw PostForMeInvalidDataException("`$name` is invalid, received $this")
        }
